...
class NativeCommandHandlerResolver implements CommandHandlerResolver
{
    ...
    //phpcs:disable
    public function __construct(protected Container $container = new NativeContainer() {
        get => $this->container;
     ...
     *
     * @param callable|CommandHandler|class-string $handler
     * @throws TypeException
     */
    public function bindHandler(string $commandName, callable|CommandHandler|string $handler): void
    {
        ...
        if (is_callable(value: $handler)) {
            $this->bindHandler(commandName: $commandName, handler: new CallableCommandHandler($handler));
            return;
        }
